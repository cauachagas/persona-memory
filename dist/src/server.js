...
export function createPersonaServer(vaultArg) {
    ...
    server.tool("get_persona_context", "Retorna um contexto compacto para inicialização da sessão com a identidade, heurísticas ativas, crenças atuais e fronteiras de aprendizado.", {}, async () => {
        try {
    ...
    server.tool("search_memory", "Executa busca léxica, filtragem por metadados e expansão de grafo (1-hop) em conceitos, heurísticas, crenças, projetos e eventos.", {
    ...
    // Tool 3: get_memory
    server.tool("get_memory", "Retorna o conteúdo integral e o frontmatter de um documento específico no cofre.", {
    ...
    server.tool("record_cognitive_event", "Permite ao agente registrar um evento cognitivo (aprendizado, mudança de entendimento ou evidência). O documento é criado como status: draft e registrado no log.md e Git, sem alterar crenças estáveis sem validação humana.", {
...
export async function runServer(vaultArg) {
    const config = getConfig(vaultArg);
